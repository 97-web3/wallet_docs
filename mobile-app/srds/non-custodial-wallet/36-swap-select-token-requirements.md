...
用户进入闪兑功能的入口页面，选择要兑换的源代币和目标代币。支持链内代币兑换，采用 DEX 聚合器获取最优汇率。非托管模式下，交易将直接使用用户钱包的私钥进行签名。
...
| **ID** | FR-36-05 |
| **描述** | 验证选择并进入闪兑详情页 |
| **验收标准** | 1. **假设** 源代币和目标代币都已选择, **当** 点击下一步, **则** 跳转至闪兑详情页 (37)<br>2. **假设** 未选择完整, **当** 点击下一步, **则** 提示"请选择代币"<br>3. **假设** 源代币余额为0, **当** 点击下一步, **则** 提示"余额不足" |
| **优先级** | 必须 |
