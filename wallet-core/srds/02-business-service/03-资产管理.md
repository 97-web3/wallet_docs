...
```json
{
  "success": true,
  "price": "0.1012",
  "stale": true,                    // 数据源不可用时为 true
  "stale_since": "2025-12-10T09:55:00Z",
  "last_updated": "2025-12-10T09:50:00Z"
}
```
...
- 价格数据优先从 Redis 缓存读取
- 缓存未命中或过期时调用外部 API
- 外部 API 失败时使用过期缓存 (最多 5 分钟前的数据)
- 返回过期数据时需标记 `stale: true`
- 前端可根据 `stale` 标记显示提示信息
