...
> 详细权限矩阵请参考 [07-admin-service/06-角色与权限](../07-admin-service/06-角色与权限.md)
...
用户 UID 采用统一格式: `U{YYYYMMDD}{4位随机字符}`

| 组成部分 | 说明 | 示例 |
|----------|------|------|
| 前缀 | 固定字符 "U" | U |
| 日期 | 注册日期 YYYYMMDD | 20251210 |
| 随机码 | 4 位大写字母/数字 | ABCD |
...
- **唯一性保证**: 生成时检查数据库，冲突则重新生成
- **字符集**: 随机码使用 `A-Z` 和 `0-9`，排除易混淆字符 (I, O, 0, 1)
- **重试机制**: 最多重试 3 次，失败则使用 UUID 后 8 位
- **存储**: 存储于 `users.uid` 字段，建立唯一索引
