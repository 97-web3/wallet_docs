...
- 预设选项: 0.1%, 0.5%, 1%, 3%
- 最小允许: 0.1% (0.001)
- 最大允许: 50% (0.50) - PRD 要求支持高波动场景
- 自定义滑点时 `is_custom` 为 true

> **PRD 对齐**: 滑点范围为 0.1% ~ 50%，预设选项为 0.5%/1%/3%。最大 50% 用于高波动代币场景。
...
**业务规则**:
- 滑点值必须在 0.001 (0.1%) 到 0.50 (50%) 之间
- 超出范围返回错误码 `SWAP_INVALID_SLIPPAGE` (70020)
- 配置持久化保存，下次闪兑时自动应用
- 滑点 > 5% 时前端应显示风险提示

> **Admin 配置**: 管理员可通过后台调整滑点范围，详见 [07-admin-service/05-Swap配置](../07-admin-service/05-Swap配置.md)
...
| 默认滑点 | 0.5% | - | 用户未设置时的默认值 |
| 最小滑点 | 0.1% | - | 允许设置的最小滑点 |
| 最大滑点 | 50% | - | 允许设置的最大滑点 (PRD 要求) |
| 预设选项 | 0.5%/1%/3% | - | 前端预设选项 |
| 报价有效期 | 30秒 | - | 报价超时时间 |
| 最小兑换金额 | 10 USDT | - | 等值最小金额 |
| 最大兑换金额 | 100,000 USDT | - | 单笔最大金额 |
| 每日限额 | 500,000 USDT | - | 单用户每日累计限额 |
| 手续费率 | 0.3% | 0-10% | 平台收取的手续费 |
| 高价格影响阈值 | 5% | - | 超过此阈值需用户确认 |

> 配置项可由管理员在后台调整，详见 [07-admin-service/05-Swap配置](../07-admin-service/05-Swap配置.md)
