...
│  • 需要手动启用后才能使用              │
│  • 同一网络只能有一个活跃地址          │
│  • 地址需通过网络特定的格式校验（详见《[地址与校验规范](./98-地址校验规范.md)》） │
│                                        │
│         [取消]    [添加]               │
│                                        │
└────────────────────────────────────────┘
...
**功能描述**: 手动调整 Vault 余额（需审批，Maker-Checker）

**用户角色**: Finance（发起）/ Admin（审批与生效）
...
│                                        │
│  ⚠️ 审批规则：                         │
│  • Finance 可发起，Admin 审批与生效    │
│  • >$10,000 需要两名不同审批人（双重审批）│
│                                        │
│         [取消]    [提交调整]           │
│                                        │
└────────────────────────────────────────┘
...
|----------|----------|
| < $1,000 | 单人审批（审批人≠发起人） |
| $1,000 - $10,000 | 单人审批（审批人≠发起人） |
| > $10,000 | 双重审批（两名不同审批人） |

#### 5.3 调整记录

调整记录会显示在网络卡片的「操作记录」区域：
...
| AdjustmentHistoryTable | Table | 调整记录表格 |
...
| 提交调整 | Toast "调整已提交"（需审批时）或 "调整已生效" | Alert 显示错误原因 |
...
**可见性**:
- 当前活跃钱包的 nonce、未确认交易数（pending 队列）
- 最近 24h 失败/替换/加速记录

**执行控制（权限受限）**:
- 对 pending 交易执行“加速（更高费率替换）/取消（0 值自转替换）”
- Gas 策略上限来自系统设置；所有操作记录审计日志
