...
转账管理的薪资导入模型要求 `employeeId/uid` 等字段（见 `wallet_docs/admin-web/transfer/01-数据模型与字段.md`），但当前“用户管理”中员工身份/业务组仍为规划项（见 `wallet_docs/admin-web/03-用户管理.md`）。
...
│ 员工目录与映射                                             [导入] [新增] │
├──────────────────────────────────────────────────────────────────────────┤
│ 筛选: [状态▼] [是否已绑定UID▼] [关键词(工号/姓名/UID)]                 │
├──────────────────────────────────────────────────────────────────────────┤
│ │工号│姓名│状态│钱包UID│最近更新│操作(查看/编辑/绑定/解绑)│            │
└──────────────────────────────────────────────────────────────────────────┘
```
...
- 若导入行包含 `employeeId`：
  - 优先从员工目录查询 `employeeId → wallet_uid`
...
- **Excel 导入**：支持 HR 导出格式（工号/姓名/状态/UID(可选)）
